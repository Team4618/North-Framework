...
public class Main extends Application implements Consumer<ConnectionNotification> {
    ...
    public static SocketAddress robotAddress = null;
    public static String connectedTo = null;
    public static long lastConnectAttempt = 0;
        ...
        while(true) {
            try {
                ...
                if(connectedTo != null) {
                    ...
                    //TODO: put a timer on the ping
                    /*
                    JSONObject pingPacket = new JSONObject();
                    pingPacket.put("Type", "Ping");
                    channel.send(ByteBuffer.wrap(pingPacket.toString().getBytes(Charset.forName("UTF-8"))), robotAddress);
                    */
                }

                //TODO: check last packet time
                if(false) {
                    connectedTo = null;
                    robotAddress = null;
                }

                long timeSinceLastConnectAttempt = System.currentTimeMillis() - lastConnectAttempt;
                if((connectedTo == null) && (timeSinceLastConnectAttempt > 1000)) {
                    ...
                    try {
                        channel.send(ByteBuffer.wrap(connectPacket.toString().getBytes(Charset.forName("UTF-8"))), new InetSocketAddress("10.46.18.2", 5801));
                        channel.send(ByteBuffer.wrap(connectPacket.toString().getBytes(Charset.forName("UTF-8"))), new InetSocketAddress("169.254.90.77", 5801));
                        channel.send(ByteBuffer.wrap(connectPacket.toString().getBytes(Charset.forName("UTF-8"))), new InetSocketAddress("172.22.11.2", 5801));
        ...
        try {
            ...
            switch((String) json.get("Type")) {
                case "Welcome": {
                    ...
                    connectedTo = name;
                    robotAddress = sender;
                    System.out.println("Connected to " + connectedTo + " @ " + robotAddress);
                    try {
